...
public class E_use_of_java_break {
	...
	public static void main(String[] args) {
		...
		for (int j = 1; j <= 5; j++) { 
			// if the condition meet the criteria, the code will move to the condition body, and the break feature will work
			if(j==3) {				
				break;
		...
		for (int k = 1; k <=5; k+=2) { 
			if (k==4) {
				break; // if the condition doesn't meet, compiler will not move to condition body, so there will be no use of 'break'
			}
